...
#' @param check_comma If \code{TRUE} (the default), will check to see that each bib entry ends with a comma after its final field. If \code{FALSE}, this check will not be performed.
...
checkGrattanReport <- function(path = ".",
  ...
  check_spelling(filename,
                 ...
                 known.correct = c(grattanReporter::grattan_correctly_spelled_words,
                                   grattanReporter::grattan_CORRECTLY_SPELLED_WORDS_CASE_SENSITIVE),
                 known.correct.fixed = grattanReporter::correctly_spelled_words_fixed_txt,
